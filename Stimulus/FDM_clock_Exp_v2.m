...
clear
clc
cd('/Volumes/ProjectExt/Han/Jumping/');
addpath('/Volumes/ProjectExt/Han/Jumping/');
...
nRep = 30; % number of condition replication
...
%% Noise texture
tmp = [];
for cnt = 1:300, tmp.tmp{cnt} = round( rand(rect.size(2), rect.size(1)) );
end % for 1024*768 screen
...
resp_rect = [ptb.xCenter ptb.yCenter]*[1 0 1 0; 0 1 0 1] + deg2pix(rsp_size)*[-1 -1 1 1];
...
for cnt = 1:height(expT)
    ...
    dioRect = [ 0 0 repmat(round(rect.rect(4)*0.3),1,2) ];% upper left corner for photodiode
